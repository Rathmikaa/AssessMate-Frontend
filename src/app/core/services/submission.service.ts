...
export class SubmissionService {
  ...
  submit(req: SubmitAssessmentRequest): Observable<SubmissionResult> {
    return this.http
      .post<ApiResponse<SubmissionResult>>(`${API_BASE_URL}/candidate/submissions`, req)
      .pipe(
  ...
  getMyResults(): Observable<SubmissionSummary[]> {
    return this.http
      .get<ApiResponse<SubmissionSummary[]>>(`${API_BASE_URL}/candidate/results`)
      .pipe(
        map((res) => res.body ?? []),
        catchError((err) => this.toFailure(err)),
      );
  ...
  getDetail(submissionId: number): Observable<SubmissionDetail> {
    return this.http
      .get<ApiResponse<SubmissionDetail>>(`${API_BASE_URL}/candidate/results/${submissionId}`)
      .pipe(
        map((res) => this.unwrap(res)),
        catchError((err) => this.toFailure(err)),
      );
